...
// HashTable is adaptee class with KEY, VALUE strings
class PassServer : HashTable<std::string, std::string> {
  public:
    ...
    bool addUser(std::pair<std::string,  std::string> & kv); // add a new username and password.  The password passed in is in plaintext, it should be encrypted before insertion. The pair should not be added if the username already exists in the hash table.
    ...
    bool find(const std::string & user); // check if a user exists (if user is in the hash table).
